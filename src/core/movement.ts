...
import type { Army, MatchPlayer, MatchState, Tile } from './types';
...
/** Whether a region may be entered by this player right now. */
export function canEnterRegion(state: MatchState, player: MatchPlayer, regionId: number): boolean {
  const region = state.regions.find((r) => r.id === regionId);
  if (!region) return true;
  if (region.lock.type === 'none') return true;
  return player.unlockedRegions.includes(regionId);
}

export function canEnterTile(state: MatchState, army: Army, tile: Tile): boolean {
  ...
  const domainOk = domains.some((d) => terrain.passableBy.includes(d));
  if (!domainOk) return false;
  return canEnterRegion(state, player, tile.regionId);
}

/** Movement point cost to step into `tile`. Infinity means "not traversable". */
export function moveCost(state: MatchState, army: Army, tile: Tile): number {
  if (!canEnterTile(state, army, tile)) return Infinity;
  const domains = armyDomains(army);
  // Pure air formations ignore ground movement costs entirely.
  if (domains.includes('air') && domains.length === 1) return 1;
  const terrain = TERRAINS[tile.terrain];
  let cost = tile.road ? Math.max(ROAD_MOVE_COST, terrain.moveCost * ROAD_MOVE_COST) : terrain.moveCost;
  if (tile.controlledBy && tile.controlledBy !== army.owner) {
...
export function findPath(state: MatchState, army: Army, to: HexId): PathResult {
  ...
  while (frontier.size > 0) {
    ...
    for (const nId of neighborIds(q, r)) {
      ...
      const step = moveCost(state, army, tile);
