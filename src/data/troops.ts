...
export interface TroopDefinition {
  ...
  /**
   * Barracks level that unlocks this troop. The barracks decides WHICH troops
   * exist for you at all (Clash of Clans style); the Academy decides how high
   * their level can go, and in-match research adds the doctrine on top.
   */
  barracksLevel: number;
  /** Training time in days at the city. Resolved on day advance. */
  trainDays: number;
...
export function troopUpgradeCost(def: TroopDefinition, level: number): ResourceCost {
  ...
  if (level >= 2) out.titanium = level - 1 + (def.barracksLevel >= 4 ? 1 : 0);
  return out;
}

export const MAX_TROOP_LEVEL = 5;

/** Troops a barracks of the given level unlocks, in roster order. */
export function troopsUnlockedAtBarracks(level: number): TroopDefinition[] {
  return TROOP_IDS.map((id) => TROOPS[id]).filter((def) => def.barracksLevel === level);
}

/** Every troop a barracks of this level (or lower) has made available. */
export function troopsForBarracks(level: number): TroopDefinition[] {
  return TROOP_IDS.map((id) => TROOPS[id]).filter((def) => def.barracksLevel <= level);
}
